...
MERGED_df_ <- allData[,c(16:46,51:60,62:63)]
...
FileOut <- file(paste("/mnt/9e33c15a-acae-4445-96c2-5ebe80a1c36a/MIREIA/TFM_REGRESSIONS/2_RESULTS/3_RESULTS_FINAL/3.1_LINEAR/LIN_REGR_",Name,".csv"), "w")
for (i in 1:length(names(MERGED_df_))){
  ...
  names(nWork)
    ...
    lm1 <- lm(VARIABLE_INTERES~(GRS+as.factor(SEX)+EDAD+as.factor(ORIGEN)+ZSCORE),data=nWork)
...
for (z in (1:length(VariablesInteres))){
  ...
  for (i in 1:length(names(MERGED_df_))){
    ...
    lm1 <- glm(formula= VARIABLE_INTERES~(GRS+SEX+EDAD+ORIGEN+ZSCORE),data=nWork, family=binomial (link=logit))
  ...
  #  cat(capture.output(summary(lm1), file="Metabolyc_Syndrome_PGS000308_BMIadj_Fasting_Insulin.txt"))
    ...
    cadena1<-paste(names(MERGED_df_)[i], beta.1, se.1, ci.lo.1, ci.hi.1, t_value.1, pvalu.1,odr.1, sep='\t')
    cat( cadena0, cadena1,file=FileOut, sep="\n")
  }
  close(FileOut)
}
