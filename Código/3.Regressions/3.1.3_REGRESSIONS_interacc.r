...
MERGED_df_ <- allData[,c(15,16,17,53,54,64,37:46,21:23,18,19,52,74,24,25,26,27,55,56)]
MERGED_df_ <- as.data.frame(apply(MERGED_df_,2,as.numeric))
MERGED_df_ <- MERGED_df_[,sort(names(MERGED_df_))]
...
# Acelerometria
 ...
 i=8
  ...
  SEX <- as.factor(allData$sex_T2)
  ZSCORE <- allData$bmi_zscore
  ORIGEN <- as.factor(allData$Origen_T1)
...
#for (i in 1:length(names(MERGED_df_))){
  ...
  ACCELEROMETRY <- MERGED_df_[,j]

  FileOut <- file(paste("/mnt/9e33c15a-acae-4445-96c2-5ebe80a1c36a/MIREIA/TFM_REGRESSIONS/2_RESULTS/3_RESULTS_FINAL/3.4_INTERACTIONS/INTERACC_",names(MERGED_df_)[j],"_",Name,"_",names(MERGED_df_)[i],".csv",sep=""), "w")


  Work <- rbind(VARIABLE_INTERES,ZSCORE,SEX,GRS,ORIGEN,EDAD,TALLA,ACCELEROMETRY)
  Work <- t(Work)
  nWork<-na.omit(Work)
...
#  lm1 <- lm(VARIABLE_INTERES~(ACCELEROMETRY*GRS+SEX+EDAD+ORIGEN),data=nWork)
 lm1 <- lm(VARIABLE_INTERES~(ACCELEROMETRY*GRS+SEX+EDAD+ORIGEN+ZSCORE),data=nWork)
# lm1 <- lm(VARIABLE_INTERES~(ACCELEROMETRY*GRS+SEX+EDAD+ORIGEN+ZSCORE+TALLA),data=nWork)
  #  summary(lm1)
#    cat(capture.output(summary(lm1), file="Metabolyc_Syndrome_PGS000308_BMIadj_Fasting_Insulin.txt"))
    coef0.lm1 <- summary(lm1)$coeff
    beta.1  <- coef0.lm1[length(coef0.lm1[,1]),1]
    se.1    <- coef0.lm1[length(coef0.lm1[,1]),2]
    t_value.1 <- coef0.lm1[length(coef0.lm1[,1]),3]
    pvalu.1  <- coef0.lm1[length(coef0.lm1[,1]),4]
 #   odr.1   <- exp(beta.1)
    ...
    cadena1<-paste(names(MERGED_df_)[i], (beta.1), (se.1), (ci.lo.1), (ci.hi.1), (t_value.1),pvalu.1, sep='\t')
    cat( cadena0, cadena1,file=FileOut, sep="\n")
    cat( cadena0, cadena1, sep="\n")
close(FileOut)


    if ( pvalu.1 < 0.05 ) {
    #  ggplotRegression(lm1)
      pdf(paste("/mnt/9e33c15a-acae-4445-96c2-5ebe80a1c36a/MIREIA/TFM_REGRESSIONS/2_RESULTS/3_RESULTS_FINAL/3.4_INTERACTIONS/INT_",names(MERGED_df_)[j],"_",Name,"_",names(MERGED_df_)[i],"_regression_plot.pdf",sep="_"))
      library(interactions)
      interact_plot(lm1,pred = GRS, modx = ACCELEROMETRY, plot.points = TRUE,
        x.label = Name,
        y.label = names(MERGED_df_)[i]
      )
...
for (z in (1:length(VariablesInteres))){
  ...
  FileOut <- file(paste("/mnt/9e33c15a-acae-4445-96c2-5ebe80a1c36a/MIREIA/TFM_REGRESSIONS/2_RESULTS/3_RESULTS_FINAL/3.2_LOGISTIC/LOG_REGR_",Name,".csv"), "w")
  for (i in 1:length(names(MERGED_df_))){
    ...
    nWork<-na.omit(Work)
    nWork <- as.data.frame(nWork)
    nWork[,c(2,4,6)] <- as.data.frame(lapply(nWork[,c(2,4,6)],as.numeric))
    nWork[,c(1,3,5)] <- as.data.frame(lapply(nWork[,c(1,3,5)],as.factor))
    dim(nWork)
    names(nWork)

    lm1 <- glm(formula= VARIABLE_INTERES~(GRS+SEX+EDAD+ORIGEN),data=nWork, family=binomial (link=logit))
