...
MERGED_df_ <- allData[,c(1:11,16:36,51:59,62,63)]
...
FileOut <- file(paste("/mnt/9e33c15a-acae-4445-96c2-5ebe80a1c36a/MIREIA/TFM_REGRESSIONS/2_RESULTS/3_RESULTS_FINAL/3.3_PHYSICAL_ACTIVITY/3.3.1_LINEAR_ACELEROM/LIN_REGR_",Name,".csv"), "w")
for (i in 1:length(names(MERGED_df_))){

  VARIABLE_INTERES <- MERGED_df_[,i]

  Work <- rbind(VARIABLE_INTERES,ZSCORE,SEX,GRS,ORIGEN,EDAD)
  Work <- t(Work)
  nWork<-na.omit(Work)
  nWork <- as.data.frame(nWork)
  nWork <- as.data.frame(lapply(nWork,as.numeric))
  nWork[,c(3,5)] <- as.data.frame(lapply(nWork[,c(3,5)],as.numeric))
  dim(nWork)
  names(nWork)
...
for (z in (1:length(VariablesInteres))){
  ...
  FileOut <- file(paste("/mnt/9e33c15a-acae-4445-96c2-5ebe80a1c36a/MIREIA/TFM_REGRESSIONS/2_RESULTS/3_RESULTS_FINAL/3.3_PHYSICAL_ACTIVITY/3.3.2_LOGISTIC_ACELEROM/LOG_REGR_",Name,".csv"), "w")
  for (i in 1:length(names(MERGED_df_))){
